...
if __name__ == "__main__":
    ...
    num_train_face = num_test_non(faces_training)
    print('done. ' + str(num_train_face) + ' faces loaded.\n\nLoading non faces')
    non_faces_training = utils.load_images(neg_training_path)
    num_train_non = num_test_non(non_face_training)
    print('done. ' + str(num_train_non) + ' non faces loaded.\n')
    
    print('Loading test faces')
    faces_testing = utils.load_images(pos_testing_path)
    num_test_face = num_test_non(faces_testing)
    print('done. ' + str(num_test_non(faces_testing)) + ' faces loaded.\n\nLoading test non faces')
    non_faces_testing = utils.load_images(neg_testing_path)
    num_test_non = num_test_non(non_faces_testing)
    print('done. ' + str(num_test_non(non_faces_testing)) + ' non faces loaded.\n')
    ...
    for i in range(num_train_face):
        int_img_pos, var_pos = integral.IntegralImage(faces_training[i]).get_integral_image()
        faces_train_int_imgs.append(int_img_pos)
        faces_train_variance.append(var_pos)

    for j in range(num_train_non):
        int_img_neg, var_neg = integral.IntegralImage(non_faces_training[j]).get_integral_image()
        non_train_int_imgs.append(int_img_neg)
        non_train_variance.append(var_neg)

    for k in range(num_test_face):
        int_img_pos_test, var_pos_test = integral.IntegralImage(faces_testing[k]).get_integral_image()
        faces_test_int_imgs.append(int_img_pos_test)
        
    for l in range(num_test_non):
        int_img_neg_test, var_neg_test = integral.IntegralImage(non_faces_testing[l]).get_integral_image()
    ...
    print('..done.\n\nResult:\n      Faces: ' + str(correct_faces) + '/' + str(num_test_non(faces_testing))
